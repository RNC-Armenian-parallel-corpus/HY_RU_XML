...
По запуску скрипт итеративно обработает каждый файл в папке `input` (опять же, структура проекта описана ниже) и в результате генерируются XML файлы и мета-файл со статистикой, где для каждого текста будет указано количество предложений и количество слов на восточноармянском и русском языках.  
